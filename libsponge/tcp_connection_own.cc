...
void TCPConnection::real_send(bool syn) {
    if (syn || _syn_sent) {
        ...
        if ((_receiver.stream_out().input_ended() && _sender.stream_in().eof() && _sender.bytes_in_flight() == 0) &&
            (_linger_after_streams_finish == false || _time_since_last_segment_rcvd >= 10 * _cfg.rt_timeout)) {
            _active = false;
        }
    }
}
...
void TCPConnection::segment_received(const TCPSegment &seg) {
    if (_active) {
        _time_since_last_segment_rcvd = 0;  // reset
        ...
        */
        ...
        if (TCPState::state_summary(_sender) == TCPSenderStateSummary::SYN_SENT && (seg.payload().size() > 0) &&
            seg.header().ack)
            return;  //
        ...
        */
        ...
        if (seg.header().rst) {
            if ((!seg.header().ack) && TCPState::state_summary(_sender) == TCPSenderStateSummary::SYN_SENT)
                return;
            unclean_shutdown(false);
            return;
        }
    ...
    /*
      if (_active){
        ...
        if (_receiver.unassembled_bytes() == 0 && _receiver.stream_out().input_ended() &&
      (_sender.stream_in().eof()==false)) { _linger_after_streams_finish = false; } //check if there is a need to linger
